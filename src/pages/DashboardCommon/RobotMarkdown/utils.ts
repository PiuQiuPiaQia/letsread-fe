...
export const formatResult = (
  ...
  if (dataType && ["handwriting"].includes(dataType)) {
    if (Array.isArray(res.pages)) {
      ...
      const pageRects = res.pages.reduce(
        (pre: Record<string, any>, cur: any, idx: number) => {
          if (cur.page_id === 0) {
            isFromZero = true;
          }
            ...
            for (const line of cur.content) {
              if (line.sub_type === "handwriting") {
                ...
                if (metrics[cur.page_id]) {
                  Object.assign(
                    row,
                    lodash.pick(metrics[cur.page_id], ["angle"])
                  );
                }
  ...
  if (Array.isArray(res.detail)) {
    let isFromZero = false;
    const pageRects = res.detail.reduce(
      (pre: Record<string, any>, cur: any, idx: number) => {
        if (cur.page_id === 0) {
          isFromZero = true;
        }
        ...
        if (dataType) {
          ...
          if (
            ["formula", "handwriting"].includes(dataType) &&
            !cur.tags?.includes(dataType)
          )
            return pre;
        }
    ...
    if (Array.isArray(res.catalog?.generate)) {
      ...
      for (let index = 0; index < catalog.length; index += 1) {
        ...
        if (
          typeof item.pageNum === "number" &&
          Array.isArray(pageRects[dataIndex])
        ) {
